...
export const dbInsertReturning = (table, values, token) =>
  request(`/rest/v1/${table}`, {
    method:'POST', token,
...
async function uploadImage(bucket, file, token) {
  const extension = (file.name.split('.').pop() || 'jpg').toLowerCase().replace(/[^a-z0-9]/g, '');
  const path = `${crypto.randomUUID()}.${extension}`;
  const response = await fetch(`${SUPABASE_URL}/storage/v1/object/${bucket}/${path}`, {
    method:'POST',
    headers:{ apikey:SUPABASE_KEY, Authorization:`Bearer ${token || SUPABASE_KEY}`, 'Content-Type':file.type, 'x-upsert':'false' },
    body:file
  });
...
export const uploadLegendImage = (file, token) => uploadImage('legend-images', file, token);
export const uploadSubmissionImage = file => uploadImage('submission-images', file);
