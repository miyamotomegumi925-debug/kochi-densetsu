...
const renderPosts = posts => {
  ...
  communityGrid.innerHTML = posts.map((post,index) => {
    ...
    return `<article class="community-card">${post.image_url ? `<img class="community-image" src="${escapeHtml(post.image_url)}" alt="${escapeHtml(post.legend)}">` : ''}<div class="quest-no">QUEST ${String(posts.length-index).padStart(3,'0')}</div><div class="meta"><span>投稿</span><span>📍 ${escapeHtml(post.place)}</span></div><h3>${escapeHtml(post.legend)}</h3><dl><dt>✨ なぜ伝説？</dt><dd>${escapeHtml(post.why)}</dd><dt>🎒 会いかた</dt><dd>${escapeHtml(post.access)}</dd></dl>${videoId ? `<a class="youtube-link" href="https://www.youtube.com/watch?v=${videoId}" target="_blank" rel="noopener noreferrer"><span>▶</span><b>YouTubeで見る</b><small>動画で伝説をたしかめる →</small></a>` : ''}<footer><span>BY ${escapeHtml(post.author || '名もなき冒険者')}</span><time datetime="${escapeHtml(post.created_at)}">${new Date(post.created_at).toLocaleDateString('ja-JP')}</time></footer></article>`;
  }).join('');
};
