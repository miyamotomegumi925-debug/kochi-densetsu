import { dbSelect, dbInsert } from './supabase.js?v=20260730-2';
...
const loadPublicData = async () => {
  try {
    ...
    const { initKochiMap } = await import('./maps.js?v=20260730-3');
...
document.querySelectorAll('[data-area]').forEach(button => button.addEventListener('click', () => {
  ...
  document.querySelectorAll('[data-pin]').forEach(pin => pin.classList.toggle('hot', pin.dataset.pin === button.dataset.area));
}));
