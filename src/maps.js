...
const mapCategories = ['すべて', ...Object.keys(categorySymbols)];
const areaFocus = {
...
export function initKochiMap(items) {
  ...
  let categoryFilter = shell.querySelector('.map-category-filter');
  if (!categoryFilter) {
    categoryFilter = document.createElement('div');
    categoryFilter.className = 'map-category-filter';
    categoryFilter.setAttribute('role', 'group');
    categoryFilter.setAttribute('aria-label', '地図に表示する伝説のカテゴリ');
    categoryFilter.innerHTML = mapCategories.map(category => `<button type="button" data-map-category="${category}" aria-pressed="${category === 'すべて'}">${category}</button>`).join('');
    shell.append(categoryFilter);
  }
  let zoom = 1;
  let panX = 0;
  let panY = 0;
  ...
  const applyZoom = () => {
    ...
    pins.style.setProperty('--pin-scale', String(1 / zoom));
  ...
  shell.addEventListener('pointerdown', event => {
    if (zoom <= 1 || event.target.closest('.map-zoom-controls,.map-popup,.map-category-filter')) return;
  ...
  const render = (area = activeArea, focusArea = false) => {
    activeArea = area;
    const areaItems = area ? mapped.filter(item => item.area === area || (!item.area && area === '中央部')) : mapped;
    const visible = activeCategory === 'すべて' ? areaItems : areaItems.filter(item => item.category === activeCategory);
    ...
    pins.onclick = event => {
      ...
      const href = item.detail_url || (item.id && item.title ? `#legend-${encodeURIComponent(item.id)}` : '#zukan');
      popup.innerHTML = `<button class="map-popup-close" type="button" aria-label="閉じる">×</button>${item.image_url ? `<img class="map-popup-image" src="${escapeHtml(item.image_url)}" alt="">` : ''}<small>LEGEND No.${escapeHtml(item.legend_no || 'NEW')} / ${escapeHtml(item.category || '伝説')}</small><h3>${escapeHtml(title)}</h3><p>📍 ${escapeHtml(item.place)}</p><p>${escapeHtml(summary)}</p><a class="map-popup-detail" href="${escapeHtml(href)}">この図鑑を表示する →</a>`;
      popup.hidden = false;
      popup.querySelector('.map-popup-close').onclick = () => { popup.hidden = true; };
    };
  };

  categoryFilter.onclick = event => {
