import { dbSelect, dbInsert, dbUpdate, dbDelete, signInAdmin, refreshAdminSession } from './supabase.js?v=20260730-5';
...
$('#legend-form').onsubmit = async event => {
  ...
  try { await withSession(token => dbInsert('legends', data, token)); event.currentTarget.reset(); $('#legend-status').textContent = '✓ 登録しました'; await loadDashboard(); }
