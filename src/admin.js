import { dbSelect, dbInsert, dbInsertReturning, dbUpdate, dbDelete, signInAdmin, refreshAdminSession, uploadLegendImage } from './supabase.js?v=20260801-2';
...
function renderSettings() {
  ...
  const options = '<option value="">掲載しない</option>' + published.map(x => `<option value="${x.id}">No.${esc(x.legend_no || '---')} ${esc(x.title)}</option>`).join('');
  $('#new-legend-options').innerHTML = published.length
...
$('#legend-form').onsubmit = async event => {
  ...
  const duplicate = legends.some(item => item.title.trim() === data.title.trim() && item.place.trim() === data.place.trim());
  if (duplicate) {
    if (promotionSource) {
  ...
  try {
    ...
    form.reset();
    $('#legend-status').textContent = promotionSource ? '✓ 投稿を伝説図鑑へ移行しました。フォームをクリアしました。' : '✓ 登録しました。フォームをクリアしました。';
    promotionSource = null;
    await loadDashboard();
  }
